...
export default function EditProfile({ onclose }) {
  ...
  const [formData, setFormData] = useState(userData); // Stan formularza
    ...
    if (session?.user?.id) {
      try {
        ...
        if (response.ok) {
          ...
          onclose(); // Zamknij formularz po zakończeniu aktualizacji
        } else {
  ...
  return (
    <div className="w-[100%] h-fit mx-auto items-center flex flex-col">
      <button onClick={onclose}>
        <IoMdArrowRoundBack className="absolute top-0 left-0 ml-[29%] mt-20 text-3xl" />
      </button>
      <h1 className="text-center text-4xl mt-16">Edytuj dane</h1>
      <div className="flex w-[60%] mt-9 justify-center mx-auto">
        {userData && (
          <form
